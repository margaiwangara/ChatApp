...
if(mysqli_num_rows($owner_query) > 0)
{
    ...
    $messages = mysqli_query($conn, "SELECT * FROM messages WHERE receiver = '$owner_id' OR sender = '$owner_id'") or trigger_error("Messages query failed");
    ...
    echo json_encode(array('sender' => $senders, 'receiver' => $receivers, 'message' => $usermessages));
}
